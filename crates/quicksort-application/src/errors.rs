...
//!
//! # Design Decisions
//! - Each variant clearly indicates the source of the error (domain, repository, file system).
//! - The `Domain` variant wraps domain-level errors, preserving the original context.
//! - The `Internal` variant is reserved for truly unexpected situations (e.g., poisoned locks).
...
/// Adapters (Tauri commands, IPC handlers) map these to appropriate responses.
#[derive(Debug, Error)]
pub enum UseCaseError {
    ...
    /// A conflict occurred (e.g., file already exists and overwrite policy is Skip).
    #[error("Conflict: {0}")]
    Conflict(String),
    ...
    /// The requested operation cannot be undone (e.g., already undone, not completed).
    #[error("Operation not undoable: {0}")]
    UndoNotPossible(String),

    /// An invalid state transition was attempted (e.g., completing a non-executing operation).
    ...
    /// An unexpected internal error occurred. Use sparingly.
