...
use quicksort_domain::{
    AbsolutePath, DuplicateCheckMode, FolderId, OperationSource, OperationType,
};
...
pub struct OperationCommand {
    ...
    /// Populated by the initiating adapter; falls back to `Api` for legacy
    /// callers that do not send it (#15, release 0.2.6).
    #[serde(default)]
    pub source: OperationSource,

    /// Traceability id linking user intent to operation execution.
    /// Generated by the client at intent time; the server fills in with
    /// `Uuid::new_v4` when absent (#15, release 0.2.6).
...
impl Default for OperationCommand {
    /// Blank command used mostly by tests that only set the fields they
    /// care about via struct-update syntax (`..OperationCommand::default()`).
    fn default() -> Self {
        Self {
            operation_type: OperationType::Move,
