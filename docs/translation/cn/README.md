...
- **重复文件检测** — 按名称、大小或 SHA-256 内容哈希进行预操作检查，按操作配置
- **操作历史** — 完整的文件操作审计记录，支持撤销（撤销会跳过已被手动移动的文件）
- **背景操作队列** — 持久化批量队列，由 worker 逐条执行并报告进度
- **可配置默认值** — 默认操作类型、覆盖策略和重复检查模式（右键菜单遵循配置的模式）

### 界面

- **文件夹编辑器** — 在简洁的 GUI 中添加、重命名、切换收藏
- **全部文件夹选择器** — 搜索、筛选并从整个文件夹库中选择；在专用窗口中打开
- **Total Commander 插件** — 安装、启用/禁用和管理 WCX 压缩包插件（支持拖放）
- **文件搜索** — Everything 风格的查询语法（`SearchQuery`），支持内容/名称筛选
- **命令面板** — 交互式 CLI 风格输入 (Ctrl+Shift+Space)，使用 Everything 搜索语法
- **事件日志** — 实时后端和前端日志记录与筛选（ring buffer），加上队列任务进度
- **操作状态栏** — 一览 COM 注册和队列状态
- **富数据表格** — 历史/队列/日志表格，支持列宽调整、拖放和多列排序（react-data-grid）
- **深色主题** — 内置明暗配色方案，琥珀色强调，与 Windows 同步
- **系统托盘** — 后台运行，保持任务栏整洁

### 智能安装

- **零安装部署** — 单个可执行文件，首次启动自动注册 COM 服务器
- **便携模式** — 便携 ZIP（exe + COM DLL + icon）；无需管理员权限，一切在用户空间运行
- **NSIS 安装程序** — 按用户安装（无需管理员权限），双语言（EN/RU），卸载时恢复 Windows shell

## 愿景

QuickSort 正在发展为全功能、跨平台文件管理器：

- **跨平台** — 目前支持 Windows（NSIS 安装程序 + 便携 ZIP）；Linux（通过 Python IPC 客户端支持 Nautilus/Dolphin/Thunar/LXQt）和 macOS（Automator Services，共享文件桥接）将基于同一 Rust/Tauri 核心实现（ADR-017）
- **命令行界面** — 交互式 Everything 风格搜索语法，用于高级文件查询、筛选和排序（命令面板已支持此语法）
- **批量操作** — 将后台队列扩展为大规模文件移动，支持暂停/恢复和流式复制（ADR-020 fast copy manager）
- **智能文件分析** — 基于内容的重复检测、文件类型识别和元数据索引
- **Total Commander 生态** — 在已实现的 WCX 基础上，支持 WDX（内容）、WFX（虚拟文件系统）和 WLX（文件列表）插件
- **文件整理器** — 按类别规划和预览文件整理，支持隔离区（ADR-022）
- **原生 WASM 插件 + 应用市场** — 超越 TC 格式的插件运行时
- **SQLite** — 随着数据层增长，将存储从 JSON 迁移到 SQLite
