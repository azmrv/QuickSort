...
- **Обнаружение дубликатов** — проверка по имени, размеру или SHA-256 хешу содержимого, настраиваемая для каждой операции
- **История операций** — полный аудит всех файловых операций с возможностью отмены (отмена пропускает файлы, уже перемещённые вручную)
- **Очередь фоновых операций** — устойчивая пакетная очередь с воркером; операции выполняются по очереди с отчётом о прогрессе
- **Настраиваемые параметры** — тип операции по умолчанию, политика перезаписи, режим проверки дубликатов (контекстное меню следует настроенному режиму)

### Интерфейс

- **Редактор папок** — добавление, переименование, избранное в удобном GUI
- **Выбор из всех папок** — поиск, фильтрация и выбор из всей библиотеки папок; открывается в отдельном окне
- **Плагины Total Commander** — установка, включение/отключение и управление плагинами архивов WCX (drag-and-drop)
- **Поиск файлов** — синтаксис запросов в стиле Everything (`SearchQuery`) с фильтрацией по содержимому/имени
- **Командная палитра** — интерактивный CLI-ввод (Ctrl+Shift+Space) с синтаксисом поиска Everything
- **Лог событий** — логирование бэкенда и фронтенда в реальном времени с фильтрацией (кольцевой буфер), а также прогресс задач в очереди
- **Заголовок статуса операции** — регистрация COM и состояние очереди на виду
- **Таблицы данных** — таблицы Истории/Очереди/Лога с изменением ширины столбцов, drag-and-drop и многосортировкой (react-data-grid)
- **Тёмная тема** — встроенные светлая и тёмная схемы с янтарными акцентами, синхронизация с Windows
...
- **Портативный режим** — портативный ZIP (exe + COM DLL + иконка); не требует прав администратора, всё работает в пространстве пользователя
- **Установщик NSIS** — установка для текущего пользователя (без прав администратора), двуязычный (EN/RU), чистая деинсталляция с восстановлением оболочки Windows

## Видение

QuickSort развивается в полноценный кроссплатформенный файловый менеджер:

- **Кроссплатформенность** — сегодня Windows (установщик NSIS + портативный ZIP); Linux (Nautilus/Dolphin/Thunar/LXQt через Python IPC-клиенты) и macOS (Automator Services, мост через общий файл) на одном ядре Rust/Tauri (ADR-017)
- **Командная строка** — интерактивный ввод с синтаксисом поиска в стиле Everything для расширенных файловых запросов, фильтрации и сортировки (командная палитра уже говорит на этом синтаксисе)
- **Пакетные операции** — масштабирование фоновой очереди на крупные перемещения с паузой/возобновлением и потоковым копированием (ADR-020 fast copy manager)
- **Умный анализ файлов** — обнаружение дубликатов по содержимому, распознавание типов файлов и индексация метаданных
- **Экосистема Total Commander** — плагины WDX (содержимое), WFX (виртуальная ФС) и WLX (листер) рядом с реализованными WCX-архивами
- **Органайзер файлов** — планирование и предпросмотр организации по категориям с карантином (ADR-022)
- **Нативные WASM-плагины + Marketplace** — среда выполнения плагинов за рамками форматов TC
- **SQLite** — миграция хранения с JSON по мере роста уровня данных
...
[QuickSort License (No-Sell)](LICENSE) — свободное использование и модификация;
использование в коммерческих организациях разрешено при соблюдении условий об
атрибуции и share-alike; продажа программы или производных работ, а также
извлечение прибыли из её распространения запрещены (полные условия — в LICENSE).
